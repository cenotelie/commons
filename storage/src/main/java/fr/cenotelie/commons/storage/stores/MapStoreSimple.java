...
class MapStoreSimple {
    ...
    public static MapStoreSimple create(ObjectStore store) {
        ...
        try (Access access = store.access(entry, true)) {
            // write header
            access.writeLong(ObjectStore.KEY_NULL);
    ...
    public boolean tryPut(long key, long valueNew) {
        return compareAndSet(key, ObjectStore.KEY_NULL, valueNew);
    }
    ...
    public boolean tryRemove(long key, long valueOld) {
        return compareAndSet(key, valueOld, ObjectStore.KEY_NULL);
    }
    ...
    public boolean compareAndSet(long key, long valueOld, long valueNew) {
        ...
        try {
            inspect(null, accessCurrent, head, valueNew != ObjectStore.KEY_NULL);
            while (true) {
                // inspect the current node
                ...
                boolean hasChanged = inspect(accessCurrent, accessChild, child, valueNew != ObjectStore.KEY_NULL);
                while (hasChanged) {
                    accessChild.close();
                    count = accessCurrent.seek(8 + 2).readChar();
                    child = getChild(accessCurrent, key, count);
                    accessChild = store.access(child, true);
                    hasChanged = inspect(accessCurrent, accessChild, child, valueNew != ObjectStore.KEY_NULL);
                }
                // rotate
                accessFather = accessCurrent;
    ...
    private boolean doCompareAndReplace(Access accessCurrent, int index, char count, long entryValue, long valueOld, long valueNew) {
        if (entryValue != valueOld)
            // oops, compare failed
            return false;
        if (valueNew != ObjectStore.KEY_NULL) {
            // this is a replace
    ...
    private boolean doInsert(Access accessCurrent, char count, long key, long valueOld, long valueNew) {
        if (valueNew != ObjectStore.KEY_NULL) {
            ...
            if (valueOld != ObjectStore.KEY_NULL)
                // expected a value
                return false;
            ...
            return (valueOld == ObjectStore.KEY_NULL);
        }
    }
    ...
    private boolean inspect(Access accessFather, Access accessCurrent, long current, boolean isInsert) {
        ...
        if (isInsert && count >= 2 * N) {
            ...
            long neighbourLeft = ObjectStore.KEY_NULL;
            long neighbourRight = ObjectStore.KEY_NULL;
            long previousValue = ObjectStore.KEY_NULL;
            int indexCurrent = -1;
            ...
            if (neighbourRight != ObjectStore.KEY_NULL) {
                boolean freeRight;
            ...
            if (neighbourLeft != ObjectStore.KEY_NULL) {
                boolean freeRight;
    ...
    private void splitRootLeaf(Access accessCurrent, long current, char count) {
        ...
        try (Access accessRight = store.access(right, true)) {
            ...
            accessRight.writeLong(ObjectStore.KEY_NULL);
        }
        // rewrite the root node
    ...
    public void clear(List<Entry> entries) {
        ...
        try (Access accessHead = store.access(head, true)) {
            ...
            accessHead.writeLong(ObjectStore.KEY_NULL);
        }
    }
    ...
    private class EntriesIterator implements Iterator<Entry> {
        ...
        public EntriesIterator() {
            ...
            this.currentCount = 0;
            this.currentNeighbour = ObjectStore.KEY_NULL;
            this.nextIndex = 0;
        ...
        public Entry next() {
            ...
            nextIndex++;
            if (nextIndex >= currentCount && currentNeighbour != ObjectStore.KEY_NULL) {
                // go to next node
